...
- デプロイ: Cloudflare Workers（`wrangler.jsonc` — static assets + `ASSETS` / `IMAGES` binding、custom domain brand.kumo.productions）。main への push で Workers Builds（Git 連携: kumoproductions/brand、`pnpm run build` → `npx wrangler deploy`、Node は `.node-version`）が自動デプロイし、他ブランチは Preview になる。手動は `pnpm run deploy`（`pnpm deploy` は pnpm 組み込みコマンドと衝突する）。静的ファイルが先に解決され、該当がないパスだけ Worker に落ちる。
